...
## `skip_docs_on_install`

Determines whether or not to skip the generation of rdoc/ri documentation
when RVM calls the `gem install` command. In the interest of speed, these
docs will not be generated by default. To re-enable the documentation
generation set:

    node[:rvm][:skip_docs_on_install] = "disable"
