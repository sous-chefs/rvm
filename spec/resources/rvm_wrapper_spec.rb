...
describe 'rvm_wrapper' do
  let(:chef_run) do
    ChefSpec::SoloRunner.new(platform: 'ubuntu', version: '22.04', step_into: ['rvm_wrapper']) do |node|
      node.automatic['platform_family'] = 'debian'
    end.converge('test::plugin')
  end
  ...
  before do
    ...
    allow_any_instance_of(RvmCookbook::RvmHelper).to receive(:wrapper_exists?).and_return(false)
